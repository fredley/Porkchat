...
function addTwitterHover(el){
    if (!window.pc_options.hover){ return };
    if(el.attr('href').indexOf('://twitter.com/') > -1){
        let show = true
        el.hover(function(){
            show = true
            $.ajax({
                url:$(this).parent().find('.permalink').attr('href'),
                method:'GET',
                success:(data) => {
                    show && $('body').append($('<div>').addClass('img-hover').append($(data).find('.message.highlight .content')));
                }
            })
        }, function(){
           $('.img-hover').remove();
